...
with tf.Session() as session:
    # Initiate all variables within the graph
    ...
    for epoch in range(learningEpochs):
        ...
        # Every 5 runs, print out the accuracy
        if epoch % 5 == 0:
            trainingCost, trainingSummary = session.run([cost,summary],feed_dict={x: xScaledTraining, y: yScaledTraining})
            testingCost, testingSummary = session.run([cost, summary],feed_dict={x: xScaledTest, y: yScaledTest})

            # Write to the tensorboard log files
            trainingWriter.add_summary(trainingSummary, epoch)
            testingWriter.add_summary(testingSummary, epoch)

            print(epoch, trainingCost, testingCost)
